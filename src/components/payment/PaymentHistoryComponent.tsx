        ...
        <div className='absolute px-2 overflow-x-hidden overflow-y-auto bg-white rounded-lg shadow-2xl min-w-64 right-5 top-20 h-96'>
            {Array.isArray(payments) ? (
                payments.length > 0 ? (
                    ...
                    <div className='flex items-center justify-center h-full'>No hay pagos realizados.</div> // Mensaje cuando payments está vacío
                )
            ) : (
                <div className='flex items-center justify-center h-full'>Cargando...</div> // Mensaje en caso de que payments no sea un array
            )}
        </div>
